...
confint(gas_price_boot, level = 0.95)
```
The theory is unsupported by the data because the distribution for difference is mean price between gas stations with stoplights and gas stations without stoplights is centered around zero meaning overall despite variability the stoplight variable has no affect on the price of gas.
...
#make confidence range
lower <- round((quantile(gas_price_boot$diffmean, probs = 0.025)), 3)
higher <- round((quantile(gas_price_boot$diffmean, probs = 0.975)), 3)
```
The difference in price between gas stations with direct highway access and gas stations without direct highway access is somewhere between `r lower` and `r higher`, with 95% confidence.
...
higher <- round((quantile(gas_price_boot$diffmean, probs = 0.975)), 3)
```
The difference in price between Shell brand gas stations and non-Shell brand gas is somewhere between `r lower` and `r higher`, with 95% confidence.
